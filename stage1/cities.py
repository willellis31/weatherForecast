...
def getCities():
    ...
    # filter the large file to only ones that have a population field
    filterCityFile()
    # ensure there is only data in each line
    data = [line.strip() for line in open("filteredcities.txt", 'r')]
...
def filterCityFile():
    '''Function to filter cities from the file to ones that have a population field '''
    # remove any unwanted white space from each line
    data = [line.strip() for line in open("world_cities/worldcitiespop.txt", 'r')]
    # split each line by the ,'s present
    cities = [line.split(",") for line in data]
    # move the lines foward by one
    cities = cities[1:]
    # create new array
    filteredcities = []
    # check for a population value
    for line in cities:
        if line[4] != '':
            filteredcities.append(line)
    # write the shortened list to a new file
